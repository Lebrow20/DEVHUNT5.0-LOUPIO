...
const ChatBot = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const requestedPage = detectPageRequest(inputValue);
      ...
      if (userInput.includes('dessin') || userInput.includes('dessiner') || userInput.includes('colorier') || userInput.includes('crayon') || userInput.includes('couleur') || userInput.includes('studio art')) {
        responseText = `J'adore dessiner ! ${pagesRef.current.dessin.description} ${
      ...
      // Temps
        ...
        responseText = `Il est ${timeString} ! ${
            ...
            <div className="flex items-center">
              ...
              <button
                ...
                className={`bg-gradient-to-r from-purple-500 to-pink-500 text-white px-4 py-2 rounded-r-md ${
                  (isLoading || willRedirect) ? 'opacity-50 cursor-not-allowed' : 'hover:from-purple-600 hover:to-pink-600'
                }`}
                disabled={isLoading || willRedirect}
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
