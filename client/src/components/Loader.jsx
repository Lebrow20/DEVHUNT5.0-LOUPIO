...
const Loader = () => {
  ...
  const [currentText, setCurrentText] = useState("🔮 Préparation des histoires...");

  const progressTexts = {
    0: "🔮 Préparation des histoires...",
    25: "📚 Chargement des quiz...",
    50: "🎭 Réveil de Tom...",
    75: "✨ Finalisation...",
    100: "🎉 Prêt !"
  };
    ...
    const interval = setInterval(() => {
      setProgress(prev => {
        ...
        if (newProgress <= 25) {
          ...
          setCurrentText(progressTexts[25]);
        } else if (newProgress <= 75) {
          setCurrentText(progressTexts[50]);
        } else if (newProgress < 100) {
          setCurrentText(progressTexts[75]);
        } else {
          setCurrentText(progressTexts[100]);
        }
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-8">
      ...
      <div className="w-80 mb-6">
        <div className="bg-white/30 backdrop-blur-sm rounded-full h-4 shadow-inner overflow-hidden">
          <div 
            className="h-full bg-gradient-to-r from-yellow-400 via-blue-500 to-white rounded-full transition-all duration-300 ease-out shadow-lg"
        ...
        <div className="text-center mt-2">
          <span className="text-2xl font-bold bg-gradient-to-r from-yellow-600 via-blue-600 to-gray-700 bg-clip-text text-transparent">
            {progress}%
          </span>
        </div>
      </div>
      ...
      <div className="text-center">
        <p className="text-xl font-semibold text-gray-700 transition-all duration-500 ease-in-out transform">
          {currentText}
        </p>
        {progress === 100 && (
